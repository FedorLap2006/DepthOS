...
static void output_copy(void (*output)(void *context, const char *data,
                                       size_t sz),
...
static int format_int(void (*output)(void *context, const char *data,
                                     size_t sz),
...
static int
format_rich_int(void (*output)(void *context, const char *data, size_t sz),
                void *context, unsigned long v, struct vformat_flags flags) {
  int sw = 0;
  int aw = 0;
  int vw = 0;
  ...
  if (flags.signed_value && (flags.sign != SIGN_DEFAULT || (long)v < 0))
    sw = 1;
  if (flags.alternate) {
    if (flags.radix == 8 && v)
      aw = 1;
  ...
  if (flags.width && sw + aw + w1 + vw < flags.width_value) {
  ...
  if (flags.signed_value) {
    if ((long)v < 0) {
      ...
      output(context, flags.sign == SIGN_PLUS ? "+" : " ", 1);
    }
  }
...
static void
format_string(void (*output)(void *context, const char *data, size_t sz),
              void *context, const char *p, struct vformat_flags flags) {
  int w0 = 0;
  int w1 = 0;

  if (flags.width && flags.width_value > flags.precision_value) {
  ...
  while (*fmt) {
    ...
    if (state == LENGTH) {
      ...
      case 'p':
        ...
        flags.alternate = 1;
        flags.radix = 16;
        format_rich_int(output, context, (unsigned long)pvoid, flags);
        break;
      case 'c':
        c = va_arg(ap, int);
        flags.precision = 1;
